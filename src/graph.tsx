...
export class Graph extends React.Component<GraphProps, GraphState> {
  ...
  public render(): JSX.Element | null {
      ...
      if (edge.points.length > 1) {
        for (let i = 1; i < edge.points.length; i++) {
          ...
          if (i === 1) {
            ...
            if (!sourceNode) {
              throw new Error(
                `Graph definition is invalid. Cannot get node by '${edgeInfo.v}'.`
              );
            }
    ...
    return (
      <div style={{ width: "1000px", height: "1000px" }} className={css.root}>
        {graph
          .nodes()
          ...
          .map((node, i) => (
            ...
            >
              ...
              <div
                className={css.icon}
                style={{ background: node.statusColoring }}
              >
                {node.icon}
              </div>
            </div>
  ...
  private _addDiagonalSegment(
    ...
    const length =
      Math.sqrt(Math.pow(xStart - xEnd, 2) + Math.pow(yStart - yEnd, 2)) + 0.5;
  ...
  private _ensureXIsWithinNode(node: dagre.Node, originalX: number): number {
    ...
    const rightmostAcceptableLoc =
      node.x + node.width - this.LEFT_OFFSET - this.EDGE_X_BUFFER;
    ...
    // in from the left end of the node.
    const leftmostAcceptableLoc =
      node.x - this.LEFT_OFFSET + this.EDGE_X_BUFFER;
