...
function buildDag(
  ...
  templates: Map<string, { nodeType: nodeType; template }>,
    ...
    (template.dag.tasks || []).forEach((task) => {
      ...
      if (child) {
        if (child.nodeType === "dag") {
          ...
          nodeLabel =
            parseTaskDisplayName(child.template.metadata) || nodeLabel;
...
export function createGraph(workflow): dagre.graphlib.Graph {
  ...
  if (!workflow.spec.entrypoint) {
    throw new Error(
      "Could not generate graph. Provided Pipeline had no entrypoint."
    );
  }
  ...
  if (graph.nodeCount() === 0) {
    const entryPointTemplate = workflowTemplates.find(
      (t) => t.name === workflow.spec.entrypoint
    );
...
export function transitiveReduction(
  graph: dagre.graphlib.Graph
): dagre.graphlib.Graph | undefined {
  ...
  result.nodes().forEach((node) => {
    ...
    result
      .successors(node)
      ?.forEach((successor: any) => dfs_with_removal(successor, node));
  });
  return result;
}

export function compareGraphEdges(
  graph1: dagre.graphlib.Graph,
  graph2: dagre.graphlib.Graph
) {
  return (
    graph1
      .edges()
