export function parseTaskDisplayName(metadata?): string | undefined {
  if (!metadata?.annotations) {
    return undefined;
  }
  const taskDisplayName =
    metadata.annotations["pipelines.kubeflow.org/task_display_name"];
...
export function parseTaskDisplayNameByNodeId(
  nodeId: string,
  workflow?: any
): string {
