...
import { ExecutionHelpers } from "./MlmdUtils";
...
export default class WorkflowParser {
  public static createRuntimeGraph(workflow, executions): dagre.graphlib.Graph {
    ...
    (Object as any).values(workflowNodes).forEach((node: any) => {
      const nodeLabel = parseTaskDisplayNameByNodeId(node.id, workflow);

      let mlmdState: any | undefined;
      ...
      if (!hasFinished(node.phase as NodePhase) && !this.isVirtual(node)) {
        ...
        g.setEdge(node.id, node.id + runningNodeSuffix, {
          color: color.weak,
          isPlaceholder: true
        });
      }
  ...
  public static getParameters(workflow?: any): any[] {
  ...
  public static getNodeInputOutputArtifacts(
    workflow?: any,
    nodeId?: string
  ): Record<"inputArtifacts" | "outputArtifacts", Array<KeyValue<any>>> {
    type ParamList = Array<KeyValue<any>>;
  ...
  public static getNodeVolumeMounts(
    workflow: any,
    nodeId: string
    ...
    if (tmpl && tmpl.container && tmpl.container.volumeMounts) {
      volumeMounts = tmpl.container.volumeMounts.map((v) => [
        v.mountPath,
        v.name
      ]);
    }
  ...
  public static getNodeManifest(
    ...
    if (
      tmpl &&
      tmpl.resource &&
      tmpl.resource.action &&
      tmpl.resource.manifest
    ) {
  ...
  public static loadAllOutputPaths(workflow: any): StoragePath[] {
    return this.loadAllOutputPathsWithStepNames(workflow).map(
      (entry) => entry.path
    );
  }
    ...
    if (workflow && workflow.status && workflow.status.nodes) {
      Object.keys(workflow.status.nodes).forEach((n) =>
        this.loadNodeOutputPaths(workflow.status.nodes[n]).map((path) =>
          outputPaths.push({
            stepName: workflow.status.nodes[n].displayName,
            path
          })
        )
      );
  ...
  public static getOutboundNodes(graph: any, nodeId: string): string[] {
    let outbound: string[] = [];
    ...
    for (const outboundNodeID of node.outboundNodes || []) {
      ...
      if (outNode && outNode.type === "Pod") {
        ...
        outbound = outbound.concat(
          this.getOutboundNodes(graph, outboundNodeID)
        );
      }
    }
    return outbound;
  ...
  public static isVirtual(node: any): boolean {
    return (
      (node.type === "StepGroup" ||
        node.type === "DAG" ||
        node.type === "TaskGroup" ||
  ...
  public static getWorkflowError(workflow: any): string {
    if (
      workflow &&
      workflow.status &&
      workflow.status.message &&
      (workflow.status.phase === NodePhase.ERROR ||
        workflow.status.phase === NodePhase.FAILED)
    ) {
